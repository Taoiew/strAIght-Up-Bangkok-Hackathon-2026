...
export function NavigationRail({ isDark, isProfileOpen, onNewChat, onOpenSupport, onToggleTheme, onToggleProfile, onLogout }: Props) {
  return (
    <nav className="z-50 flex h-full w-16 shrink-0 flex-col items-center justify-between border-r border-gray-200 bg-white py-6 transition-colors dark:border-chat-border dark:bg-chat-base md:w-20">
      ...
      <div className="relative flex flex-col items-center gap-4">
        ...
        <button type="button" aria-label="Open profile menu" aria-expanded={isProfileOpen} onClick={onToggleProfile} className="flex h-10 w-10 items-center justify-center rounded-full bg-brand text-sm font-semibold text-white shadow-sm ring-brand-light transition hover:ring-2">C</button>
        {isProfileOpen && (
          <div className="absolute bottom-0 left-16 z-[60] flex w-48 flex-col rounded-xl border border-gray-100 bg-white py-2 shadow-xl dark:border-chat-border dark:bg-[#1e1e1e]">
            <div className="mb-2 border-b border-gray-50 px-4 py-2 dark:border-gray-800"><p className="text-sm font-semibold text-gray-900 dark:text-white">Chisapat P.</p><p className="mt-0.5 text-xs font-medium text-green-500">● Logged In</p></div>
